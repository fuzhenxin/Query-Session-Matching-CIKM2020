...
def loss_class(x, y, num_classes=2, is_clip=True, clip_value=10):
    '''From info x calculate logits as return loss.

    Args:
        x: a tensor with shape [batch, turn_sess]
        num_classes: a number
...
def attention(
    ...
    drop_prob=None, q_time=None, k_time=None, use_len=False):
    '''Add attention layer.
    Args:
...
def block(
    ...
    is_layer_norm=True, 
    is_mask=True, mask_value=-2**32+1,
    drop_prob=None, q_time=None, k_time=None, use_len=False, no_mask=False):
    ...
    att = attention(Q, K, V, 
                    ...
                    drop_prob=drop_prob, q_time=q_time, k_time=k_time, use_len=use_len)
    if is_layer_norm:
...
def get_inference_weight(x1, x1_mask, x2, x2_mask, initializer_opt):

    with tf.variable_scope('self_stack_mem_weight1', reuse=tf.AUTO_REUSE):
        role_emb1 = tf.get_variable(name="role_emb1", shape=[x1.shape[1], 25], dtype=tf.float32, initializer=initializer_opt)
        x1_weight = layers.block(x1, x1, x1, Q_lengths=x1_mask, K_lengths=x1_mask, use_len=True)

        role_emb1 = tf.expand_dims(role_emb1, axis=0)
        role_emb1 = tf.tile(role_emb1, [tf.shape(x1_weight)[0], 1, 1])
        x1_weight = tf.concat([role_emb1, x1_weight], axis=2)

        x1_weight = tf.layers.dense(x1_weight, 1, activation=tf.nn.sigmoid, name="fnn", kernel_initializer=initializer_opt)
        x1_weight = tf.squeeze(x1_weight, [2,])

    with tf.variable_scope('self_stack_mem_weight2', reuse=tf.AUTO_REUSE):
        role_emb2 = tf.get_variable(name="role_emb2", shape=[x2.shape[1], 25], dtype=tf.float32, initializer=initializer_opt)
        x2_weight = layers.block(x2, x2, x2, Q_lengths=x2_mask, K_lengths=x2_mask, use_len=True)

        role_emb2 = tf.expand_dims(role_emb2, axis=0)
        role_emb2 = tf.tile(role_emb2, [tf.shape(x2_weight)[0], 1, 1])
        x2_weight = tf.concat([role_emb2, x2_weight], axis=2)

        x2_weight = tf.layers.dense(x2_weight, 1, activation=tf.nn.sigmoid, name="fnn", kernel_initializer=initializer_opt)
        x2_weight = tf.squeeze(x2_weight, [2,])

    return x1_weight, x2_weight

def local_inference(x1, x1_mask, x2, x2_mask, weight1=None, weight2=None, initializer_opt=None):
    ...
    attention_weight_2 = attention_weight - tf.reduce_max(attention_weight, axis=2, keepdims=True)
    if weight2 is not None:
        weight2 = tf.expand_dims(weight2, axis=1)
        attention_weight_2 = attention_weight_2+weight2
    attention_weight_2 = tf.exp(attention_weight_2)
    ...
    if weight1 is not None:
        weight1 = tf.expand_dims(weight1, axis=2)
        attention_weight_1 = attention_weight_1+weight1
...
def bilstm_layer(input_data, num_layers, rnn_size, keep_prob=1., initializer_opt=None):
    #rnn_size = int(rnn_size)
    #return bilstm_layer_cudnn(input_data, num_layers, rnn_size, keep_prob=1.), None
    #print(rnn_size)
    #return bigru_layer_cudnn(input_data, num_layers, rnn_size, keep_prob=1.), None
    """Multi-layer BiLSTM
    Args:
    ...
    for layer in range(num_layers):
        with tf.variable_scope('bilstm_{}'.format(layer), reuse=tf.AUTO_REUSE):
            ...
            cell_fw = tf.contrib.rnn.LSTMCell(
                rnn_size, initializer=initializer_opt)
            ...
            states = tf.concat([states[0][1], states[1][1]], axis=1)
...
def bigru_layer_cudnn(input_data, num_layers, rnn_size, keep_prob=1.):
    """Multi-layer BiLSTM cudnn version, faster
    Args:
        input_data: float32 Tensor of shape [seq_length, batch_size, dim].
        num_layers: int64 scalar, number of layers.
